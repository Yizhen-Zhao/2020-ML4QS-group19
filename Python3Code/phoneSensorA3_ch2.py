...
from Chapter2.CreatDatasetPhoneA3 import CreateDataset
...
DATASET_PATH = Path(sys.argv[1] if len(sys.argv) > 1 else './datasets/phoneA3/')
RESULT_PATH = Path('./intermediate_datafiles/')
RESULT_FNAME = sys.argv[2] if len(sys.argv) > 2 else 'phoneSensorsA3_ch2.csv'
...
for milliseconds_per_instance in GRANULARITIES:
    ...
    dataset.add_numerical_dataset('Location.csv', 'Time', ['latitude','longitude','height','velocity','direction','horizontalAccuracy','verticalAccuracy'], 'avg', 'loc_mobile_')  # unit:()
    ...
    dataset.add_event_dataset('labels_phone.csv', 'label_start', 'label_end', 'label', 'binary')
    ...
    DataViz.plot_dataset(dataset, ['acc_mobile_', 'gyr_mobile_', 'mag_mobile_', 'prox_mobile_distance', 'loc_mobile_', 'label'],
